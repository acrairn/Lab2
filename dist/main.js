...
function guessCheck(){
    ...
    if(userGuess === secretNumber){
        isCorrect.textContent = 'Correct! You are the winner!';
        isCorrect.style.backgroundColor = 'greenyellow';
        gameOver();
    } else if(guessCount === 10){
        isCorrect.textContent = 'GAME OVER! Try harder next time.';
        gameOver();
        ...
        if(userGuess < secretNumber){
            hint.textContent = 'That is too low (ಠ⌣ಠ)';
        }else{
            hint.textContent = 'That is too high (ಥ⌣ಥ)';
        }
    }
